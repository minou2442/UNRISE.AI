...
app.post('/api/predict-major', async (req, res) => {
  try {
    ...
    const prompt = `
كمستشار توجيه مهني متخصص في التعليم العالي في الجزائر، حلل هذا الملف الشخصي للطالب:
...
بناءً على هذا الملف الشامل، اقترح أفضل 3 تخصصات جامعية مناسبة له في الجزائر.
لكل توصية، قدم:
1. اسم التخصص
2. شرحًا موجزًا (من 2 إلى 3 جمل) لماذا يناسب هذا التخصص ملفه الشخصي
3. مسارين مهنيين ممكنين في الجزائر لهذا التخصص
